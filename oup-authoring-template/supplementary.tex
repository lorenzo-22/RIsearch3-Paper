...
sequences searched against human chromosome-X cDNA targets (Ensembl transcripts
$\geq 80$\,nt), on an AMD EPYC~9554 server using 32 threads. Both tools were
supplied with pre-built indexes and identical search parameters---seed scheme,
energy threshold ($-10$\,kcal\,mol$^{-1}$), extension length, mismatch count and
output format---so that only search time is compared and index construction is
excluded. Wall-clock time was recorded with Hyperfine (one warm-up plus three
timed runs, output cleared before each run). In all panels the Rust
implementation is labelled \texttt{RIsearch\_RS}; the full commands and parameter
grid are given in the Supplementary Methods.
...
\caption{%
  ...
  Fig.~\ref{fig:risearch-comparison} use gzip for both tools, and RIsearch's
  zstd advantage over RIsearch2 (Table~\ref{tab:risearch-speedup}) is reported
  separately.%
}
...
\begin{table}[ht]
\centering
\caption{Speed-up of RIsearch over RIsearch2 on the chromosome-X workload
  (32 threads). Ranges span the seed schemes 2:7, 2:7/5, 1:12/6, 1:12/7,
  1:12/8, 4, 5 and 6; ``seed-and-extend'' aggregates extension lengths
  10--20\,nt and ``seed-only'' uses extension~0. Matched rows use gzip output
  for both tools; because RIsearch2 has no zstd mode, the zstd rows compare
  RIsearch (zstd) against RIsearch2 (gzip).}
\label{tab:risearch-speedup}
\small
\begin{tabular}{lcc}
\hline
Comparison (32 threads) & Median speed-up & Range \\
\hline
Seed-and-extend, matched gzip output    & $1.9\times$ & $1.5$--$2.3\times$ \\
Seed-and-extend, RIsearch zstd output   & $2.3\times$ & $1.7$--$2.6\times$ \\
Seed-only, RIsearch zstd output         & $2.9\times$ & $2.1$--$3.3\times$ \\
\hline
\end{tabular}
\end{table}

\subsection*{RIOT pipeline benchmark}
...
The RIsearch/RIsearch2 search benchmark
(Supplementary Figures~\ref{fig:risearch-comparison}--\ref{fig:risearch-compression-scaling},
Table~\ref{tab:risearch-speedup}) used 63 siRNA guide sequences as queries
against a chromosome-X target set of Ensembl human cDNA transcripts filtered to
$\geq 80$\,nt. All runs were executed on an AMD EPYC~9554 server (32 threads);
input files were staged on local disk to remove NFS latency, and timing was
collected with Hyperfine (one warm-up run followed by three timed runs, with the
output directory cleared before each run). To isolate search performance, each
tool was given a pre-built index of the same target and run with matched
parameters; index construction was excluded from all timings.
...
mismatch counts 0--3; output formats \texttt{p1}--\texttt{p4}; and, for RIsearch
only, gzip and zstd output compression. Thread scaling was measured for seed
scheme~6 at 1, 2, 4, 8, 16 and 32 threads. The energy threshold was
$-10$\,kcal\,mol$^{-1}$ for extension $\geq 5$\,nt and $0$ for seed-only runs
(extension $<5$\,nt, for which no dynamic-programming extension is performed).

The two search commands, shown for seed scheme 2:7/5 with extension 20\,nt on
32 threads, were:
...
\noindent Both tools used the \texttt{t99} energy matrix and produced binding-site
output. Matched head-to-head comparisons used \texttt{-{}-compress gzip}; the
zstd comparisons (Figs.~\ref{fig:risearch-compression},
\ref{fig:risearch-compression-scaling}) substitute \texttt{-{}-compress zstd}.
Because RIsearch2 emits only gzip-style output, the RIsearch-versus-RIsearch2
figures compare gzip output for both tools; with RIsearch's faster zstd output
the advantage widens further (Table~\ref{tab:risearch-speedup}).
