...
RIsearch reimplements the seed-and-extend search of
RIsearch2~\citep{alkan2017risearch2} in Rust. To quantify the resulting
speed-up, both tools were benchmarked on a common workload: 63 siRNA guide
sequences searched against a chromosome-X target set of Ensembl human cDNA
transcripts filtered to $\geq 80$\,nt, on an AMD EPYC~9554 server. Both tools
were supplied with pre-built indexes of the same target and run with identical
search parameters, so that only search time is compared and index construction
is excluded. Input files were staged on local disk to remove NFS latency, and
wall-clock time was recorded with Hyperfine (one warm-up run followed by three
timed runs, with the output directory cleared before each run). In all figures
the Rust implementation is labelled \texttt{RIsearch\_RS}.

The parameter grid comprised seed schemes 2:7, 2:7/5, 1:12/6, 1:12/7, 1:12/8 and
the singletons 4, 5, 6, 12 and 16; extension lengths 0, 5, 10, 15 and 20\,nt;
mismatch counts 0--3; and output formats \texttt{p1}--\texttt{p4}. Thread scaling
was measured for seed scheme~6 at 1, 2, 4, 8, 16 and 32 threads. The energy
threshold was $-10$\,kcal\,mol$^{-1}$ for extension $\geq 5$\,nt and $0$ for
seed-only runs (extension $<5$\,nt, for which no dynamic-programming extension is
performed). The two search commands, shown for seed scheme 2:7/5 with extension
20\,nt on 32 threads, were:
...
With matched gzip-compressed output, RIsearch runs about twice as fast as
RIsearch2 on the canonical seed-and-extend schemes
(Fig.~\ref{fig:risearch-comparison}; median $1.9\times$, up to $2.3\times$),
the advantage growing with extension length as the Gotoh dynamic-programming
stage comes to dominate runtime (Fig.~\ref{fig:risearch-extension}) and holding
across thread counts (Fig.~\ref{fig:risearch-scaling}). RIsearch additionally
offers zstd-compressed output, which RIsearch2 lacks; substituting zstd
(Figs.~\ref{fig:risearch-compression}, \ref{fig:risearch-compression-scaling})
widens the margin over RIsearch2 to a median of $2.3\times$ on seed-and-extend
workloads and up to ${\sim}3\times$ on seed-only searches.
...
RIsearch builds its suffix-array index in parallel across cores with
libsais~\citep{libsais}, whereas RIsearch2's libdivsufsort construction is
single-threaded. Building the GRCh38/hg38 index was benchmarked against
RIsearch2 with Hyperfine (Fig.~\ref{fig:indexing}). When the index is written to
disk, RIsearch is already ${\sim}1.5\times$ faster single-threaded ($0.66\times$
the RIsearch2 time) and ${\sim}2.2\times$ faster at 16 threads ($0.45\times$);
when the index is instead kept in memory, the parallel suffix-array construction
is up to ${\sim}5.6\times$ faster ($0.18\times$ at 16 threads), showing that disk
I/O, not computation, dominates the write-to-disk case.
...
(Fig.~\ref{fig:accessibility}, Table~\ref{tab:acc-bench}). RIOT completes the
full-genome fold $\sim$34\% faster (8\,h\,46\,m vs.\ 13\,h\,12\,m) and stores the
resulting profiles in $\sim$42\% less disk space (111.8\,GB vs.\ 192.6\,GB), the
latter because each chromosome is written as a single two-strand compressed
Parquet file rather than two uncompressed binary files. The trade-off is higher
peak resident memory ($\sim$3.6\,GB vs.\ $\sim$309\,MB, roughly $12\times$), as
the ViennaRNA Python API holds per-chromosome probability matrices in memory
rather than streaming them through a subprocess; both figures are small relative
to typical server memory, and this cost is incurred only once per genome and
folding condition.
